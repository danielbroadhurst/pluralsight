...
describe('/api', () => {
  ...
  context('When creating a list', () => {
    ...
    beforeEach(() => {
      return request.post('/api/createList')
        .then(res => {
    ...
    context('When adding items to the list', () => {
      ...
      beforeEach(() => {
        return request.post('/api/addItem').send({ listId, newItemName: item1Name })
          .then(() => request.post('/api/addItem').send({ listId, newItemName: item2Name }))
      ...
      context('When removing from a list', () => {
        ...
        beforeEach(() => {
          return request.delete(`/api/removeItem/${listId}/${listWithItems.items[0]._id}`)
  ...
  context('When creating two lists', () => {
    ...
    beforeEach(() => {
      return request.post('/api/createList').send({ name: list1Name })
    ...
    afterEach(() => {
      return request.delete(`/api/deleteList/${list1._id}`).expect(200)
    ...
    context('When adding items to both lists', () => {
      ...
      beforeEach(() => {
        return request.post('/api/addItem').send({ listId: list1._id, newItemName: workItem1 })
          .then(() => request.post('/api/addItem').send({ listId: list1._id, newItemName: workItem2 }))
          .then(() => request.post('/api/addItem').send({ listId: list2._id, newItemName: personalItem1 }))
          .then(() => request.get(`/api/list/${list1._id}`)
            .then(res => workListWithItems = res.body))
          .then(() => request.get(`/api/list/${list2._id}`)
    ...
    context('When getting the users lists', () => {
      ...
      beforeEach(() => {
        return request.get('/api/lists').expect(200)
